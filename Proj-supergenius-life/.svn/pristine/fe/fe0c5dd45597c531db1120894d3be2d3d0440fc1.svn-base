...
public class CKEditorMobile extends BaseController{
	...
	public ResponseEntity<String> uploadFlash(Map<String, Object> model, @RequestParam("CKEditorFuncNum") String funNum, @RequestParam(value="upload") MultipartFile file, HttpServletRequest request) {
		if (file.getSize() > 0) {
	        String path = FileUtil.uploadFile(file, BaseSysConf.VideoWebDataDir_Slash + ESite.life.getName());
	        String basePath = StringUtils.isEmpty(BaseWebConf.WebImgRootPath) ? request.getContextPath() : BaseWebConf.WebImgRootPath;
	 ...
	 * @return
	 */
	@RequestMapping(value = "/ajax/upload/music", method = RequestMethod.POST)
	@ResponseBody
	private String uploadmusic(Map<String, Object> model, @RequestParam MultipartFile localfile, HttpServletRequest request) {
		Map<String, Object> map = new HashMap<>();
		String localUrl = FileUtil.uploadFile(localfile, SysConf.LifeMusicPath);
