...
function submitCommentHandler(obj){
	...
	$.post(base + "/ajax/answer/{0}".format(params.fromuid), params,function(data){
...
function loadmore(){
	var url= "";
	if (type == 0) {
		url = base + "/ajax/answer/"+cid+"_"+fromuid+"_{0}".format(num);
	} else {
		url = base + "/ajax/lifeanswer/hotanswer";
	}
	$.ajax({
		type:"GET",
		url:url,
...
function getcomment(obj){
	var url = base + "/ajax/secondanswer/"+firstuid+"_"+cid+"_{0}".format(pagenum);
	$.ajax({
		type:"GET",
		url:url,
		success:function(data){
...
function deleteComment(obj){
	...
	var url = base + "/ajax/answer/delete/"+commentuid+"/"+fromuseruid+"/lifeproblem/"+useruid;
	$.ajax({
		type:"GET",
		url:url,
		success:function(data){
