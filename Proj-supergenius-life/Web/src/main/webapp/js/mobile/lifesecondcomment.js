...
function submitCommentHandler(obj){
	...
	$.post(base + "/m/ajax/my/lifecomment/"+channel+"/{0}".format(params.fromuid), params,function(data){
		afterSubmitHandler(data, obj);
		var tempuid = $("#articlecommentcount").html();
		$("#articlecommentcount").html(parseInt(tempuid)+1);
	});
	if($(".commentBox").hasClass("hd")){
		$(".commentBox").removeClass("hd");
	}
}
...
function loadlast(obj){
	...
	num=1;
	var url = base + "/m/ajax/lifecomment/"+channel+"_" + cid+"_"+fromuid+"_{0}".format(num);
	if(temp<num){
		temp=num;
...
function loadmore(){
	...
	if("type" in window){
		if(type == 1){
			url = base + "/m/ajax/lifecomment/hotcomment";
		}
	}
	if(temp<num){
		temp=num;
...
function showcomment(obj){
	...
	if(display == "true"){
		...
		$(obj).parent().parent().siblings("#second").css("display","none");
...
function getcomment(obj){
	var url = base + "/m/ajax/lifecomment/secondComment_"+firstuid+"_"+cid+"_{0}".format(pagenum);
	$.ajax({
		type:"GET",
		url:url,
		success:function(data){
			if(data.trim() == ""){
				...
				$(obj).parent().parent().siblings("#second").html(data);
				initPage();
				$(obj).parent().parent().siblings("#second").css("display","block");
				pagenum+=1;
				prizeBindEvent();
...
function deleteComment(obj){
	...
	var url = base + "/m/ajax/lifecomment/delete/"+commentuid+"/"+fromuseruid+"/"+useruid;
	$.ajax({
		type:"GET",
		url:url,
		success:function(data){
