...
[#list list as item]
	<li>
		...
		[#if item.ismember==1]
			[#if me??]
        		[#if me.getIsUser() || item.user.uid == me.uid]href="${base}/m/videoDetail/${item.cid?c}/${item.uid}"[#else]href="javascript:;" class="normalUsertip"[/#if]
        	[#else]
        		href="javascript:;" class="pop-up"
        	[/#if]
		[#else]
			[#if me??]
        		href="${base}/m/videoDetail/${item.cid?c}/${item.uid}"
        	[#else]
        		href="javascript:;" class="pop-up"
        	[/#if]
		[/#if]
