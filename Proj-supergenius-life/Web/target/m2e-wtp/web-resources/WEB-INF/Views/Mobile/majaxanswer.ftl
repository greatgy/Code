		...
		<li class="commentitem">
			...
			<div class="commentitemRt replay_comment" data-uid="${item.uid}" data-touseruid="${item.fromuseruid}"  data-tousername="[@p.PrivateString content="${fromUserName}" /]"  data-useruid="${item.useruid}">
				...
				<div class="wordBottom">
					<span class="qstime">${(item.createtime).toString("M月dd日")}</span>
					<div class="qsCommentOperate">
						...
						[#else]
							<a data-reply="replyopen" class="reply" href="${base}/m/login?cid=${cid?c}&pcid=${pcid}"><img src="${baseimg}/imgs/default/discuss.png"><span id="commentcount">${item.commentcount}</span></a>
						[/#if]
			...
			<!--以下是二级评论部分-->
			<ul id="second" class="qsReplyList hd" ></ul>
