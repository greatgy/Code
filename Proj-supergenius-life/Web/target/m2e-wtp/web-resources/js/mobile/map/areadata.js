...
strVarCity += '                          <div id="" style="width: 388px; position:relative;">';
...
function selectProvince(type, con, isremove) {
    ...
    if (type == "all") {
        ...
        var datacontinentxz = __LocalDataCities.category.continents;
        // 加载热门城市和省份
        ...
        strVarCity += ' </ul>';
        strVarCity += '</div>';
        strVarCity += '    <p class="data-title">其他</p>';
        strVarCity += '   <div class="data-list data-list-provinces">';
        strVarCity += '  <ul class="clearfix">';
        for (var i in datacontinentxz) {
            strVarCity += '<li><a href="javascript:;" data-code="' + datacontinentxz[i] + '" data-name="' + dataarrary[datacontinentxz[i]][0] + '" class="d-item"  onclick="selectProvince(\'sub\',this,\'\')">' + dataarrary[datacontinentxz[i]][0] + '<label>0</label></a></li>';
        }
        strVarCity += ' </ul>';
        strVarCity += '</div>';
    ...
    //显示下一级
        ...
        if (typeof (relations[$(con).data("code")]) != "undefined") {
            ...
            if (isremove != "remove") {
                ...
                $("#province").val("");
                if (parseInt($(con).data("code").toString().substring(0, 3)) < 350) {
                    $("#province").val($(con).data("name"));
                }
            }
            //添加内容
            strVarCity += '<ul class="clearfix">';
            ...
            if ($('a[data-code=' + $(this).data("code") + ']').length > 0) {
                ...
                if ($('a[data-code=' + $(this).data("code") + ']').attr("class").indexOf('data-all') > 0) {
                    ...
                    if ($('.data-list-provinces a[data-code=' + $(this).data("code").toString().substring(0, 3) + ']').length > 0) {
                        ...
                        console.log(2);
                         $("#province").val("");
                        if (parseInt($(this).data("code").toString().substring(0, 3)) < 350) {
                             $("#province").val($('.data-list-provinces a[data-code=' + $(this).data("code").toString().substring(0, 3) + ']').data("name"));
                        }
                    }
...
function searchdata() {
    ...
    var dataArr = [];
    for (var i in list) {
        if (i == '350' || i == '360' || i == '370' || i == '380' || i == '390' || i == '400' || i == '000') {
            continue;
        }
