...
public class MessageController extends BaseController{
	...
	public String msg_jsonp(Map<String, Object> model, HttpServletRequest request) {
		...
		int countcomment = so.getCountByUnRead(user.getUid(), EFinanceMsg.commentsarticle);
		int countnotice = so.getCountByUnRead(user.getUid(), EFinanceMsg.subscribe);
		int countsysmsg = so.getCountByUnRead(user.getUid(), EFinanceMsg.sys);
