...
//-->
$(function(){
	$("#catalogue_"+${bean.cid}).addClass("current").siblings().removeClass("current");
			
	[#if bean.cid??]
    	$("#catalogues_"+${bean.cid}).prop("checked", "checked");
    [/#if]
		});
</script>
</head>
...
<div class="contribute_box" style="width:1000px; margin-bottom:100px;clear:both;">
    ...
    <form id="contribute" class="draft" action="${base}/my/article/publish?uid=${bean.uid}" method="post" enctype="multipart/form-data">
    	<div id="form">	
	        ...
	        <label class="mb_30" >标签</label>
	        <input  class="draft_input ml_20" type="text" id="txtkeywords" name="keywords" placeholder="多个标签之间以空格隔开" value="${bean.keywords}">
            <br>
            <div>
	       ...
	       <div>
	            <label class="mb_30" ><span class="red">*</span>是否原创</label>
	       		...
	       		<input type="radio" name="isoriginal" id="isoriginal_no" value="0" /><label for="video">否</label>
	       </div>
	       <div>
	            ...
	            [#if LeftCatalogue??]
	            	...
	            	[#list LeftCatalogue as item]
	            		[#if item.cid>0]
	            			<input type="radio" name="cid" id="catalogues_${item.cid}" value="${item.cid}" [#if Leftcount == 1]checked[/#if]/><label for="${item.cid}">${item.name}</label>
	            		[/#if]
