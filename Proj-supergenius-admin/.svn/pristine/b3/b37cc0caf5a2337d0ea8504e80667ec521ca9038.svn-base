...
import com.supergenius.web.admin.gupage.helper.GupageVideoHP;
...
public class GupageVideoAdminer extends BaseController {
	 ...
	 * @return String
	 */
	@RequestMapping(value = "/gupage/gupagevideo", method = RequestMethod.GET)
	public String photo(Map<String, Object> model, HttpServletRequest request) {
		model.put(ViewKeyDict.channel, EChannel.gupagevideo.name());
		model.put(ViewKeyDict.channelname, EChannel.getName(EChannel.gupagevideo, Locale.CHINA));
	 ...
	 * @return String
	 */
	@RequestMapping(value = "/gupage/ajax/gupagevideo/list", method = RequestMethod.GET)
	@ResponseBody
	public ResponseEntity<Map<String, Object>> photo_list(Map<String, Object> model, HttpServletRequest request) {
		cloneParamsToModel(model, request);
		Map<String, Object> searchMap = GupageVideoHP.query(model);
	 ...
	 */
	@RequestMapping(value = "/gupage/ajax/gupagevideo/add", method = RequestMethod.POST)
	@ResponseBody
	public Map<String, Object> article_add(String content, String title, String summary, String[] imgdata, String keywords) {
		if (StrUtil.isNotEmpty(content) && StrUtil.isNotEmpty(title) && StrUtil.isNotEmpty(imgdata)) {
			...
			if (StrUtil.isNotEmpty(adminUid)) {
				...
				video.setCreatetime(new DateTime(DateTimeZone.forOffsetHours(8)));
				video.setUpdatetime(new DateTime(DateTimeZone.forOffsetHours(8)));
			}
			if (so.add(video)) {
				GupageVideoHP.handleCache();
				return success();
			}
		}
	 ...
	 */
	@RequestMapping(value = "/gupage/ajax/gupagevideo/delete", method = RequestMethod.GET)
	@ResponseBody
	public Map<String, Object> article_delete(String[] ids) {
		for (String id : ids) {
			...
			if (video != null) {
				...
				adminLog.setChannel(EChannel.gupagevideo.toInt());
				adminLog.setOperation(EAdminLog.deleteGupageVideo.getName());
				adminLog.setData(EAdminLog.deleteGupageVideo.getName());
				adminLog.setDesc(EAdminLog.deleteGupageVideo.getName());
	 ...
	 */
	@RequestMapping(value = "/gupage/ajax/gupagevideo/status/{status:\\d+}", method = RequestMethod.POST)
	@ResponseBody
	public Map<String, Object> article_delete(String uid, @PathVariable int status) {
		...
		if (video != null) {
			...
			adminLog.setChannel(EChannel.gupagevideo.toInt());
			adminLog.setOperation(EAdminLog.updateGupageVideo.getName());
			adminLog.setData(EAdminLog.updateGupageVideo.getName());
			adminLog.setDesc(EAdminLog.updateGupageVideo.getName());
	 ...
	 */
	@RequestMapping(value = "/gupage/ajax/gupagevideo/edit", method = RequestMethod.POST)
	@ResponseBody
	 ...
	 */
	@RequestMapping(value = "/gupage/ajax/gupagevideo/enable", method = RequestMethod.GET)
	@ResponseBody
	 ...
	 */
	@RequestMapping(value = "/gupage/ajax/gupagevideo/disable", method = RequestMethod.GET)
	@ResponseBody
	 ...
	 */
	@RequestMapping(value = "/gupage/ajax/gupagevideo/topcount", method = RequestMethod.GET)
	@ResponseBody
	public ResponseEntity<Map<String, Object>> article_topcount(Map<String, Object> model, HttpServletRequest request) {
		model.put(MapperDict.count, GupageVideoHP.getTopVideoCount());
