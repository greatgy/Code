...
<body>
    <section>
		    ...
		    <tr>
	 			<td>所属版块:
	 				...
	 				[#if catelogueList??]
						...
						[/#list]
					[/#if]
					<input name="cid" type="radio" id="catalogue66" value="274877906944" ><label for="catalogue66" >行万里路</label>
				</td>
	        </tr>
        	<tr>
                <td>是否置顶：
        ...
        [@a.datagrid  options="fitColumns:true,resizeHandle:'right',pagination:true,singleSelect:false,method:'get'"]
            ...
            <th data-options="field:'imglittle', width:15, formatter:showimglittle">标题图</th>
            <th data-options="field:'author',width:15">作者</th>
		    <th data-options="field:'cidName',width:40">所属板块</th>
		...
		[@a.viewsection]
	        ...
	        <tr class="travel"><td>作者</td><td><span set-key="author"></span></td></tr>
	        <tr class="travel"><td>SEO关键字：</td><td><span set-key="keywords"></td></tr>
	        <tr class="travel"><td>来源</td><td><span set-key="origin"></span></td></tr>
	        <tr class="travel"><td>是否置顶</td><td><span set-key="istopName"></span></td></tr>
	        <tr class="travel"><td>类别</td><td><span set-key="typeName"></span></td></tr>
	        <tr class="travel"><td>所属板块</td><td><span set-key="cataloguename"></span></td></tr>
	        <tr class="travel"><td>摘要</td><td><span set-key="summary"></span></td></tr>
	        <tr class="travel"><td>定时发布</td><td><span set-key="booktimeStr"></span></td></tr>
   	    ...
   	    [@a.editsection canuploadfile=true]
		    ...
		    <tr class="travel">
 				...
 				<td>  
	 				[#if catelogueList??]
					    [#list catelogueList as item]
					    	<input name="cids" type="checkbox" class="editcatelogue add${(item.cid)?c} sign${(item.pcid)?c}" id="edit_catalogue${(item.cid)?c}" value="${(item.cid)?c}"><label for="edit_catalogue${(item.cid)?c}" >${item.name}</label>
						[/#list]
					[/#if]
				</td>
	        </tr>
 			...
 			<td>模块:</td>
 				<td>
	 				[#if catelogueList??]
					    ...
					    [#list catelogueList as item]
						    [#assign i=i+1]
						    <input name="cids" type="checkbox" class="catelogue add${(item.cid)?c} sign${(item.pcid)?c}" id="add_catalogue${(item.cid)?c}" value="${(item.cid)?c}" [#if i==1]checked="checked"[/#if]><label for="add_catalogue${(item.cid)?c}" >${item.name}</label>
						[/#list]
					[/#if]
				</td>
        </tr>
