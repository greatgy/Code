 ...
 * 4、由于绑定的blur事件触发国语频繁，不再使用alert输出信息，而是使用隐藏域，要求隐藏域须有errorinfo="true"的属性
 * 
 * 
 */
;(function ($) {
	...
	$.fn.valid = function(options) {
		...
		regElems.bind("blur", vars, vars.regsBlurHandler);
		$("input", vars.boxs).bind("focus", vars, vars.focusHandler);
		$("input[customreg]", vars.boxs).bind("blur", vars, vars.customRegBlurHandler);
		$("input[ajaxvalid]", vars.boxs).bind("blur", vars, vars.ajaxValidBlurHandler);
		$("input[filevalid]", vars.boxs).bind("blur", vars, vars.fileValidBlurHandler);
		$("[customvalid]", vars.boxs).bind("blur", vars, vars.customValidBlurHandler);
		vars.boxs.bind("submit", vars, vars.formsValidBlurHandler);
