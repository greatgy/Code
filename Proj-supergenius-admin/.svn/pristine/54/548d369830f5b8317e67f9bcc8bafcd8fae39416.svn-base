 ...
 * @author LiJiacheng
 */
public class RecruitHP extends BaseHP {
	
	private static RecruitSO so;

	public static RecruitSO getSO() {
		if (so == null) {
			so = (RecruitSO) spring.getBean(RecruitSO.class);
		}
		return so;
	}
	...
	public static Map<String, Object> query(Map<?, ?> model) {
		...
		if (StrUtil.isNotEmpty(model.get(ViewKeyDict.title))) {
			map.put(MapperDict.title, model.get(ViewKeyDict.title).toString().trim());
		}
