    ...
    */
    function ${id}mySave() { 
        ...
        if (expertlist.length == 3) {
            ...
            $("#checkopenreplyfileerrmsg").removeClass("hd");
        }
...
<input name="${name}name" style="width:200px" readonly="readonly" placeholder="请选择三名专家！" >
...
<div id="${id}" class="easyui-window" title="选择专家" style="width:800px;height:600px" data-options="iconCls:'icon-save',modal:true,closed:true,closable:true">
    ...
    <table id="${expert}" name="${name}" class="easyui-datagrid" style="width:800px;height:380px;border:solid 1px red;" title="裁判选择列表" data-options="url:'${base}${baseAdminPath}/manager/ajax/expert/searchlist',fitColumns:true, method:'get', pagination:true, singleSelect:false">
        <thead>
            <tr>
