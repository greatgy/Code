package com.supergenius.web.admin.managernews.helper;
...
import com.supergenius.xo.managernews.rule.ManagernewsArticleRlue;
import com.supergenius.xo.managernews.rule.ManagernewsFirstArticleRule;
 ...
 */
public class ManagernewsArticleHP extends BaseHP {
	private static Logger log = LoggerFactory.getLogger(ManagernewsArticleHP.class);
	...
	public static SearchEngine getEngine() {
		if (engine == null) {
			engine = (SearchEngine) spring.getBean("managernewsEngine");
		}
		return engine;
	}
	...
	public static void initializeSearchIndex() {
		...
		delmap.put(MapperSearchDict.table, ESite.managernews.name());
	...
	public static void Cache(Article article) {
		Rule indexrule = new ManagernewsFirstArticleRule(String.valueOf(ECatalogue.index));//清除首页文章缓存
		MemcacheUtil.remove(indexrule);
		Rule bannerrule = new ManagernewsTopArticleRule(String.valueOf(article.getCid()));// 模块的轮播缓存
		MemcacheUtil.remove(bannerrule);
		Rule Firstrule = new ManagernewsFirstArticleRule();// 首页左侧缓存
		MemcacheUtil.remove(Firstrule);
		Rule cidrule = new ManagernewsArticleRlue(String.valueOf(article.getCid()));// 模块下的所有文章
		MemcacheUtil.remove(cidrule);
		Rule allrule = new ManagernewsArticleRlue(String.valueOf(0));// 所有文章的缓存
	...
	public static boolean publish() {
		...
		if (getSO().updateFields(map)) {
			...
			for (Article article : list) {
				...
				solrMap.put(MapperSearchDict.table, ESite.managernews.name());
				getEngine().add(solrMap);
			}
			log.debug("managernewsArticle publish is true");
			return true;
		}
		log.debug("managernewsArticle publish is false");
		return false;
	}
