...
function initDGData(row) {
	for(var i in row) {
		if (row[i].type == 32) {
			row[i].module = "醒世金语";
		} else if(row[i].type == 64){
			row[i].module = "企业家";
		}else if(row[i].type == 128){
			row[i].module = "职业经理人";
		}else if(row[i].type == 256){
			row[i].module = "职业夜话";
		}else if(row[i].type == 1){
			row[i].module = "财经资讯";
		}else if(row[i].type == 2){
			row[i].module = "投资资讯";
		}else if(row[i].type == 4){
			row[i].module = "并购资讯";
		}else if(row[i].type == 8){
			row[i].module = "风险资讯";
		}else if(row[i].type == 16){
			row[i].module = "科技资讯";
		}else if(row[i].type == 0){
			row[i].module = "首页";
		}
    ...
    function myButtonHandler(obj, url) {
		...
		if (objid == "disable") {
		    confirmAjax('冻结该选项', '冻结后该评论在前台将不可见，确定要冻结吗？', '${base}${baseAdminPath}/gupage/ajax/gupagecontent/status');
		}
		if (objid == "enable") {
		    confirmAjax('解冻该选项', '解冻后将在前台正常显示该评论，确认要解冻吗？', '${base}${baseAdminPath}/gupage/ajax/gupagecontent/status');
		}
	}
...
function submitHandlerForEdit(obj, dg) {
	...
	if(val == 2){
		$.get('${base}${baseAdminPath}/gupage/ajax/gupagecontent/guidecount', function(result) {
			if (result.count >= 5) {
				err("应聘指南相关推荐超过5个！");
		        return false;
			}else{
                ...
                <td>板块：
                	...
                	[#if cataloguelist??]
	                	[#list cataloguelist as item]
	                		[#if item.cid>0]
		                    	<input type="radio" id="catalogue_${item.cid}" name="type" value="${item.cid}"/><label for="catalogue_${item.cid}">${item.name}</label>
		                    [/#if]	
		                [/#list]
	                [/#if]
                </td>
   		...
   		<tr>
            ...
            <td id="addcontent">
            	[#if cataloguelist??]
            		[#assign Leftcount = 0]
                	[#list cataloguelist as item]
	                    	<input type="radio" id="addcatalogue_${item.cid}" name="type" value="${item.cid}" [#if Leftcount == 0]checked[/#if]/><label for="addcatalogue_${item.cid}">${item.name}</label>
    ...
    [@a.editsection onclicksubmit="submitHandlerForEdit(this)"]
        ...
        <tr>
            ...
            <td id="editcontent">
            	[#if cataloguelist??]
                	[#list cataloguelist as item]
	                    	<input type="radio" id="editcatalogue_${item.cid}" name="type" value="${item.cid}"/><label for="editcatalogue_${item.cid}">${item.name}</label>
	                [/#list]
                [/#if] 
            </td>
        </tr>
