...
<!--
    ...
    function ${id}Check(index, userCheck) {
        if (uids.indexOf(userCheck.uid) < 0) {
        	uids += "," + userCheck.uid;
        	names += "," + userCheck.username;
        	$("#selecteduser").append("<a href='javascript:;' index='" + index + "' onClick='removeUser(this);' uid='" + userCheck.uid + "' style='text-decoration:none; margin-right:15px;'>" + userCheck.username + "</a>");
        }
    }
    
    function ${id}UnCheck(userCheck) {
        uids = uids.replace("," + userCheck.uid, '');
        names = names.replace("," + userCheck.username, '');
...
<div id="${id}" class="easyui-window" title="选择用户" style="width:800px;height:600px" data-options="iconCls:'icon-save',modal:true,closed:true,closable:true">
    <div style="margin-left:70px">
        <table id='${usersearchtable}' class="usersearchtable">
            <tr>
                ...
                <input type="text" name="showname" style="width:200px" /></td>
    ...
    <table id="${userlist}" class="easyui-datagrid" style="width:786px;height:250px;border:solid 1px red;" title="用户选择列表" data-options="url:'${base}${baseAdminPath}/user/ajax/alluser/list',fitColumns:true,method:'get',pagination:true">
        <thead>
            <tr>
                <th data-options="field:'ck000',checkbox:true"></th>
                <th data-options="field:'usersn',width:100">用户编号</th>
                <th data-options="field:'username'">名称</th>
                <th data-options="field:'aisleName',width:100">注册方式</th>
