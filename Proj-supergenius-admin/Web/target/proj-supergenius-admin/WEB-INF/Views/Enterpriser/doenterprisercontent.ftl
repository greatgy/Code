...
$(function() {
    ...
    function myButtonHandler(obj, url) {
		...
		if (objid == "disable") {
		    confirmAjax('冻结该选项', '冻结后该评论在前台将不可见，确定要冻结吗？', '${base}${baseAdminPath}/enterpriser/ajax/enterprisercontent/status');
		}
		if (objid == "enable") {
		    confirmAjax('解冻该选项', '解冻后将在前台正常显示该评论，确认要解冻吗？', '${base}${baseAdminPath}/enterpriser/ajax/enterprisercontent/status');
		}
	}
...
function initDGData(row) {
	for(var i in row) {
		if (row[i].cid == 1) {
			...
			row[i].module = "互助合作平台";
		}else if(row[i].cid == 31){
			row[i].module = "中国制造业面临困境";
		}else if(row[i].cid == 33){
			row[i].module = "引资购商走向没落";
		}else if(row[i].cid == 35){
			row[i].module = "引资购商助力中国制造业提升";
		}else if(row[i].cid == 36){
			row[i].module = "引资购商代替招商引资";
		}else if(row[i].cid == 61){
			row[i].module = "资讯";
		}else if(row[i].cid == 62){
			row[i].module = "典型案例";
		}else if(row[i].cid == 63){
			row[i].module = "法规";
		}
	}
}
                ...
                <td>板块：
                	...
                	[#if cataloguelist??]
	                	[#list cataloguelist as item]
	                		[#if item.cid>0]
		                    	<input type="radio" id="catalogue_${item.cid}" name="cid" value="${item.cid}"/><label for="catalogue_${item.cid}">${item.name}</label>
		                    [/#if]	
		                [/#list]
	                [/#if]
                </td>
   		...
   		<tr>
            ...
            <td id="addcontent">
            	[#if cataloguelist??]
            		[#assign Leftcount = 0]
                	[#list cataloguelist as item]
	                    	<input type="radio" id="addcatalogue_${item.cid}" name="cid" value="${item.cid}" [#if Leftcount == 0]checked[/#if]/><label for="addcatalogue_${item.cid}">${item.name}</label>
    ...
    [@a.editsection onclicksubmit="submitHandlerForEdit(this)"]
        ...
        <tr>
            ...
            <td id="editcontent">
            	[#if cataloguelist??]
                	[#list cataloguelist as item]
	                    	<input type="radio" id="editcatalogue_${item.cid}" name="cid" value="${item.cid}"/><label for="editcatalogue_${item.cid}">${item.name}</label>
	                [/#list]
                [/#if] 
            </td>
        </tr>
