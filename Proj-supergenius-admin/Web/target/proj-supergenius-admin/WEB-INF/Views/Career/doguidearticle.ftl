...
<body>
    <section>
		...
		[/@a.search]
        [@a.datagrid url="${base}${baseAdminPath}/${site}/ajax/guidearticle/list"]
            ...
            <th data-options="field:'cataloguename',width:15">所属专题</th>
		...
		[@a.viewsection]
	        ...
	        <tr><td>所属专题</td><td><span set-key="cataloguename"></span></td></tr>
        ...
        [/@a.viewsection]
   	    [@a.editsection canuploadfile=true]
	        ...
	        <tr>
	         ...
	         <tr>
            <td>点赞数：</td>            
           		 <td><input class="easyui-validatebox" name="prizecount"/></td>
	        </tr>
	        <tr>
	            <td>浏览数：</td>            
	            <td><input class="easyui-validatebox" name="clickcount"/></td>
	        </tr>
	        <tr>
            <td>是否原创：</td>            
            <td>
        ...
        <tr>
			...
			<td>
 				[#if specialList??]
			    [#list specialList as item]
			    <input name="cataloguetype" type="radio" class="catelogue add${item.cid} sign${item.pcid}" id="add_catalogue${item.cid}" value="${item.cid}" [#if item.cid == 21]checked="checked"[/#if]><label for="add_catalogue${item.cid}" >${item.name}</label>
				[/#list]
				[/#if]
			</td>
        </tr>
