...
$(function() {
    ...
    function myButtonHandler(obj, url) {
		...
		if (objid == "disable") {
		    confirmAjax('冻结该选项', '冻结后该评论在前台将不可见，确定要冻结吗？', '${base}${baseAdminPath}/moralnews/ajax/moralnewsad/status');
		}
		if (objid == "enable") {
		    confirmAjax('解冻该选项', '解冻后将在前台正常显示该评论，确认要解冻吗？', '${base}${baseAdminPath}/moralnews/ajax/moralnewsad/status');
		}
	}
...
function submitHandlerForAdd(obj, dg) {
	$(obj).parents("form").form('submit', {
	    onSubmit: function(param) {
	    console.log($("#imguploadinput0").find("input[name='imgdata']").val());
	     if (($("#imguploadinput0").find("input[name='imgdata']").val() == undefined)) {
		        err("您未上传图片！");
		        return false;
       		};
       		if ($("#add_name").val().length < 1) {
		        err("您未填写名称！");
		        $("#add_name").focus();
		        return false;
	       	};
       		if ($("#add_originurl").val().length < 1) {
		        err("您未填写链接！");
		        $("#add_originurl").focus();
		        return false;
	       	};
...
function submitHandlerForEdit(obj, dg) {
	...
	if(val == 2){
		$.get('${base}${baseAdminPath}/moralnews/ajax/moralnewscontent/edit', function(result) {
		});
	}else{
...
function initDGData(row) {
	for(var i in row) {
		if (row[i].cid == 1) {
			...
			row[i].module = "职业道德缺失与批判";
		}else if(row[i].cid == 4){
			row[i].module = "职业道德与执行力";
		}else if(row[i].cid == 8){
			row[i].module = "职业道德与信用";
		}else if(row[i].cid == 16){
			row[i].module = "商业谋略与欺诈";
		}else if(row[i].cid == 32){
			row[i].module = "忠诚及其悖论";
		}else if(row[i].cid == 64){
			row[i].module = "职业道德培训与考试";
		}else if(row[i].cid == 256){
			row[i].module = "职业道德社区";
		}
	}
}
...
<body>
                ...
                <td>板块：
                	...
                	[#if cataloguelist??]
	                	[#list cataloguelist as item]
	                		[#if (item.cid)?c >0 && (item.cid)?c != 64 && (item.cid)?c !=256]
   ...
   [@a.addsection canuploadfile=true onclicksubmit="submitHandlerForAdd(this)"]
        <tr>
            ...
            <td><input type="text" class="easyui-validatebox" name="name" data-options="min:0,required:true" id="add_name"/></td>
        </tr>
        <tr>
            <td>图片:</td>
        ...
        <tr>
            <td>链接:</td>
            <td><input type="text" class="easyui-validatebox" name="originurl" data-options="min:0,required:true" id="add_originurl"/></td>
        </tr>
   		<tr>
            <td>选择类型:</td>
            <td id="addcontent">
