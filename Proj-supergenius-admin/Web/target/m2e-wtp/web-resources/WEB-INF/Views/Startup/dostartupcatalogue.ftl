...
function initordernow() {
    ...
    $(".tidbitsListul").children("li").each(function() {
        tidbitsList += $(this).children("span").data("cid") + ",";
    });
    $("#informationListnow").val(informationList.substring(0, informationList.length - 1));
    $("#tidbitsListnow").val(tidbitsList.substring(0, tidbitsList.length - 1));
}
    ...
    $(obj).parents("form").form('submit', {
        success : function(str) {
            ...
            if (result.success || result.addSuccess || result.editSuccess) {
                window.location.href = "${base}${baseAdminPath}/startup/startupcatalogue";
            } else {
...
function orderEditHandler(obj, dg, editform) {
	$.post("${base}${baseAdminPath}/startup/ajax/startupcatalogue/orderlist", function(result) {
		var data = $.parseJSON(result);
		var informationList = data.informationList;
		var tidbitsList = data.tidbitsList;
		...
		for (var i = 0 ; i < tidbitsList.length ; i++) {
			tidbitsListhtml += "<li id='information" +tidbitsList[i].cid+ "'>"
			tidbitsListhtml += "<span data-cid='"+tidbitsList[i].cid+"'></span>"
			tidbitsListhtml += tidbitsList[i].name + "<a href='javascript:;' class='nexthead'>下移</a><a href='javascript:;' class='forhead'>上移</a><br/>"
			tidbitsListhtml += "</li>"
		}
		$(".informationListul").find("li").remove(); 
		$(".informationListul").append(informationListhtml);
		$(".tidbitsListul").find("li").remove(); 
		$(".tidbitsListul").append(tidbitsListhtml);
...
<body>
    <section>
    	...
    	[@a.editsection id="setorder" namesubmit="提交" action="${base}${baseAdminPath}/${site}/ajax/${channel}/setorder"  onclicksubmit="addSubmitHandler(this)"]
		    ...
		    <input type="text" id="tidbitsListnow" name="tidbitsListnow" style="display:none" />
		    <table><tr id="datatr">
		    	<td  style="color:black">
					<div class="easyui-panel" title="创业资讯" style="width:400px;height:400">
					         <ul class="informationListul">
					         </ul>
					</div>
				</td>
				<td>
					<div class="easyui-panel" title="创业花絮" style="width:400px;">
					         <ul class="tidbitsListul">
					         </ul>
					</div>
				</td>
		    </tr></table>
