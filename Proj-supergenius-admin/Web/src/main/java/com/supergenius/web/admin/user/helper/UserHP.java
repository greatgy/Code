...
public class UserHP extends BaseHP {
    ...
    public static boolean sentInfoEmail(User user, String title, String content) {
        try {
           ...
           /* if (StringUtils.isNotEmpty(user.getMobile())) {
                SendSmsUtil.sendSmsCus(user.getMobile(), title, content);
    ...
    public static Map<String, Object> query(Map<String, Object> model) {
        ...
        if (StrUtil.isNotEmpty(model.get(ViewKeyDict.showname))) {
            map.put(MapperDict.username + MapperDict.suffix_like_key, model.get(ViewKeyDict.showname).toString());
        }
        ...
        map.put(MapperDict.ismember, EUser.consumer);
     ...
     * @author XieMing 2016-5-15 下午4:26:47
     */
    public static Map<String, Object> alluser(Map<String, Object> model) {
        Pager pager = Pager.getNewInstance(model.get("page"), model.get("rows"));
        Map<String, Object> map = getParamMap(pager, model);
        if (StrUtil.isNotEmpty(model.get(ViewKeyDict.showname))) {
            map.put(MapperDict.username + MapperDict.suffix_like_key, model.get(ViewKeyDict.showname).toString());
        ...
        if (StrUtil.isNotEmpty(model.get(ViewKeyDict.status))) {
            ...
            List<EStatus> liststa = new ArrayList<>();
            liststa.add(EStatus.enable);
            liststa.add(EStatus.wait);
            liststa.add(EStatus.start);
            map.put(MapperDict.statuslist, liststa);
		}
        if (StrUtil.isNotEmpty(model.get(ViewKeyDict.sisStudent)) && !new Boolean(model.get(ViewKeyDict.sisStudent).toString())) {
            map.put(MapperDict.type + MapperDict.suffix_no_key, EUser.studentMoral.toString());
        } else if (StrUtil.isNotEmpty(model.get(ViewKeyDict.sisStudent)) && new Boolean(model.get(ViewKeyDict.sisStudent).toString())) {
