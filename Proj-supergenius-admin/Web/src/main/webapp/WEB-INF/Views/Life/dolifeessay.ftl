...
<script type="text/javascript">
    ...
    function myButtonHandler(obj, url) {
		...
		if (objid == "disable") {
		    confirmAjax('冻结该选项', '冻结后该动态在前台将不可见，确定要冻结吗？', '${base}${baseAdminPath}/life/ajax/lifeessay/0');
		}
		if (objid == "enable") {
		    confirmAjax('解冻该选项', '解冻后将在前台正常显示该动态，确认要解冻吗？', '${base}${baseAdminPath}/life/ajax/lifeessay/1');
		}
	}
...
<body>
	<section>
		...
		[@a.datagrid]
            <th data-options="field:'fromusername',width:80">发表人</th>
            <th data-options="field:'content',width:200">动态内容</th>
            <th data-options="field:'catalogueName',width:80">所属板块</th>
            <th data-options="field:'statusName',width:80">状态</th>
            <th data-options="field:'createtimeStr',width:80">创建时间</th>
            <th data-options="field:'adminname',width:80">操作人</th>
	    [/@a.datagrid]
 	...
 	[@a.viewsection]
        ...
        <tr>
            <td>所属板块:</td>
            <td><span set-key="cataloguename"></span></td>
        </tr>
        <tr>
            <td>ip:</td>
