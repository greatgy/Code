...
function initDGData(row, time) {
	for(var i in row) {
		var pattern = "(/m){0,1}";
		if (row[i].uri.indexOf(pattern) != -1) {
			row[i].uri = row[i].uri.replace(pattern, "");
			row[i].ismobile = 1;
		} else {
			row[i].ismobile = 0;
		}
...
<body>
	...
	[@a.addsection action="${base}${baseAdminPath}/ajax/seo${site}/add"]
    	<tr>
            ...
            <input name="ismobile" type="checkbox" id="ismobile" value="1" checked><label for="ismobile">是否同步手机端</label>
            </td>
        </tr>
        <tr>
            <td>名称</td>
	...
	[@a.editsection action="${base}${baseAdminPath}/ajax/seo${site}/add"]
    	...
    	<tr>
            ...
            <input name="ismobile" type="checkbox" id="ismobile" value="1"><label for="ismobile">是否同步手机端</label>
            </td>
        </tr>
        <tr>
            <td>名称</td>
