...
<script type="text/javascript">
    ...
    function myButtonHandler(obj, url) {
		...
		if (objid == "disable") {
		    confirmAjax('冻结该选项', '冻结后该评论在前台将不可见，确定要冻结吗？', '${base}${baseAdminPath}/startup/ajax/startupcomments/status');
		}
		if (objid == "enable") {
		    confirmAjax('解冻该选项', '解冻后将在前台正常显示该评论，确认要解冻吗？', '${base}${baseAdminPath}/startup/ajax/startupcomments/status');
		}
	}
