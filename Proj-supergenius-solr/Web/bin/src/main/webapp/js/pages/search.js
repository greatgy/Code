	...
	for(var index = 0; index < data.docs.length; index++) {
		...
		if(data.docs[index]._table_ =="finance"){
			if(doc.firstTypeName == "debate") {
				...
				data.docs[index].href = "{0}/{1}/{2}".format(financebase, data.docs[index].firstTypeName, data.docs[index].oid);
			}
		}
...
function search() {
	$(".loading").removeClass("hd");
	var args;
		args = {start: 0, table: PageData.table, rows:PageData.pagesize, sort:PageData.sort};
