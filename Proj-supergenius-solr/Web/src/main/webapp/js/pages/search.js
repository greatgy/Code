...
function dataHandling(data, other) {
	...
	for(var index = 0; index < data.docs.length; index++) {
		...
		if(data.docs[index]._table_ =="managernews"){
			data.docs[index].href ="{0}/{1}/{2}/{3}".format(managernewsbase, "article", data.docs[index].firstCid, data.docs[index].oid);
		}
		if(data.docs[index]._table_ =="entrepreneur"){
			data.docs[index].href ="{0}/{1}/{2}/{3}".format(entrepreneurbase, "article", data.docs[index].firstCid, data.docs[index].oid);
		}
		if(data.docs[index]._table_ =="moralnews"){
			data.docs[index].href ="{0}/{1}/{2}/{3}".format(moralnewsbase, "article", data.docs[index].firstCid, data.docs[index].oid);
		}
		if(data.docs[index]._table_ =="life"){
			if(data.docs[index].firstTypeName == "problem"){
				data.docs[index].href ="{0}/{1}/{2}/{3}".format(lifebase, "problem", "detail", data.docs[index].cid, data.docs[index].oid);
			}else{
				data.docs[index].href ="{0}/{1}/{2}/{3}".format(lifebase, "article", data.docs[index].firstCid, data.docs[index].oid);
			}
		}
