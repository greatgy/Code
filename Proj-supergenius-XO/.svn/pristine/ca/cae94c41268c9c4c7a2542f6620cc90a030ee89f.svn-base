 ...
 */
@Json(value = {"uid", "createtime", "to", "touseruid", "fromusername", "fromuseruid", "tousername", "status", "type", "content", "ip", "title", "adminname", "createtime", "fromVisitorAvatar", "cataloguename", "fromVisitorName", "articleoid", "topicoid", "ismajor"}, ignoreTypeStrategy = Json.webStrategy, strategy = {Json.webStrategy, Json.cacheStrategy})
public class Comments extends BaseEntity implements Comparable<Comments> {
    ...
    private int articleoid;// 评论文章的oid
    private int topicoid;//评论话题的oid
    ...
    public int getArticleoid() {
        return articleoid;
    }

    public void setArticleoid(int articleoid) {
    ...
    public long getPrizecount() {
        Rule rule = new PrizeCountCommentsLifeRule(this.uid);
    ...
    public long getCommentcount() {
        Rule rule = new LifeSecondCommentCountRule(this.uid);
    ...
    public int compareTo(Comments o) {
        if (this == o) {
            ...
            if (comments.getIsmajor() == 1) {
                if (this.getCreatetime().isAfter(comments.getCreatetime())) {
                    return -1;
                }
                if (this.getCreatetime().isBefore(comments.getCreatetime())) {
                    return 1;
                }
                return 0;
            } else {
