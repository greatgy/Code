 ...
 * PuzzlesDaoImpl
 * 
 * @CreateTime 2018年1月24日--下午4:45:14
 * @author LiuBin
 */
@Component
public class PuzzlesDaoImpl extends BaseSudokuMongoDaoImpl<Puzzles> implements PuzzlesDao {

	@Override
	public List<Puzzles> getList(Map<String, Object> map) {
		return getList(map, Mongo.defaultStrategy);
	}
	
	@Override
	public List<Puzzles> getList(Map<String, Object> map, String strategy) {
		int skip = 0;
		if (map.get(BaseMapperDict.startIndex) != null) {
			skip = Integer.parseInt(map.get(BaseMapperDict.startIndex).toString());
		}
		DBObject orderby = getOrderDBObject(map); //TODO 优化此方法，当数据过大时，导致内存不够，排序报错，建立排序字段的索引
		DBCursor cursor = null;
		int limit = 0;
		if (map.get(BaseMapperDict.pageSize) != null) {
			limit = Integer.parseInt(map.get(BaseMapperDict.pageSize).toString());
			if (orderby == null) {
				cursor = getColl().find(getQuery(map)).skip(skip).limit(limit);
			} else {
				cursor = getColl().find(getQuery(map)).skip(skip).limit(limit).sort(orderby);
			}
		} else {
			if (orderby == null) {
				cursor = getColl().find(getQuery(map)).skip(skip);
			} else {
	...
	protected DBObject getOrderDBObject(Map<String, Object> map){
		...
		if (value == null) {
			return null; //如果没有排序，则返回未null，防止数据过大，导致报错
		}
	...
	private List<Puzzles> result(DBCursor cursor, String strategy) {
		if (cursor == null) {
			return null;
		}
		List<Puzzles> list = new ArrayList<>();
		if (cursor != null) {
			List<DBObject> tmpList = cursor.toArray();
			for (DBObject item : tmpList) {
				list.add((Puzzles) MapsUtil.fromMap(item.toMap(), getTClass(), strategy));
			}
		}
		return list;
	}
