package com.supergenius.xo.entrepreneur.entity;
...
import com.supergenius.xo.entrepreneur.rule.EntrepreneurSecondCommentCountRule;
import com.supergenius.xo.entrepreneur.rule.PrizeCountCommentsEntrepreneurRule;
 ...
 */
@Json(value = { "uid", "createtime", "to", "touseruid", "fromusername", "fromuseruid", "tousername", "status", "type", "content", "ip", "title", "adminname", "createtime", "fromVisitorAvatar",
		"cataloguename", "fromVisitorName" }, ignoreTypeStrategy = Json.webStrategy, strategy = Json.webStrategy)
public class Comments extends BaseEntity implements Comparable<Comments> {
	...
	private EEntrepreneurChannel channel;// 频道
	...
	public EEntrepreneurChannel getChannel() {
		return channel;
	}

	public void setChannel(EEntrepreneurChannel channel) {
		this.channel = channel;
	}

	@Json(strategy = Json.webStrategy)
	public String getChannelName() {
		return EEntrepreneurChannel.getName(channel, Locale.CHINA);
	}
	...
	public long getPrizecount() {
		Rule rule = new PrizeCountCommentsEntrepreneurRule(this.uid);
		admirecount = RedisUtil.getIncr(rule);
		return admirecount > 0 ? admirecount : 0;
	}

	public Long getCommentcount() {
		Rule rule = new EntrepreneurSecondCommentCountRule(this.uid);
