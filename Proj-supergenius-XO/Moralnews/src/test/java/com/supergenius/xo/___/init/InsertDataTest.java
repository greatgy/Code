...
public class InsertDataTest {
	...
	@Autowired
	SqlSessionFactory sqlSessionFactoryMoralnewsDB;

	Logger logger = LoggerFactory.getLogger(this.getClass());

	private static final String strDBName = "GMoralnewsDB";

	@Test
	public void insertGMoralnewsDBData() throws IOException {
		System.out.println("-----------invoking method: insertGMoralnewsDBData");
		...
		try {
			...
			conn = sqlSessionFactoryMoralnewsDB.openSession().getConnection();
			for (String sql : strs) {
	...
	@Test
	public void insertGMoralnewsDBTestData() throws IOException {
		System.out.println("-----------invoking method: insertGMoralnewsDBTestData");
		InputStream dbStream = ClassLoader.getSystemResourceAsStream(String.format("DB/InitData/%s_Test.sql", strDBName));
		String dbsql = IOUtils.toString(dbStream, SysConst.UTF8);
		// File filealtdb = new
		// File(ClassLoader.getSystemResource(String.format("DB/InitData/%s_Test_Alt-1.0.0.0.sql",
		// strDBName)).toString());
		// File dir = new File(filealtdb.getParent().replace("file:\\", ""));
		// FileFilter filter = new FileFilter() {
		// @Override
