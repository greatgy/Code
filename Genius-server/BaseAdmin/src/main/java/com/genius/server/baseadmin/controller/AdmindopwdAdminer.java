...
public class AdmindopwdAdminer extends BaseController {

	@Autowired
	private AdminSO so;
	
	@RequestMapping(value = "/admindopwd", method = RequestMethod.GET)
	public String admindopwd(Map<String, Object> model, HttpServletRequest request) {
		return "doadmindopwd";
	}
	
	@RequestMapping(value = "/admindopwd", method = RequestMethod.POST)
	@Transactional(propagation=Propagation.REQUIRED, isolation=Isolation.READ_COMMITTED)
	public String admindopwd(Map<String, Object> model, String pwd, String pwd2, HttpServletRequest request) throws Exception {
		Admin admin = so.getByAdminid(AdminHP.getAdminid());
		Map<String, String> errs = new HashMap<>();
		Map<String, String> msgs = new HashMap<>();
		if (StrUtil.isNotEmpty(pwd) && pwd.equals(pwd2)) {
			so.updateDoPwd(admin, pwd);
